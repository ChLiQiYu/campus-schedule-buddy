...
class ScheduleFragment : Fragment(R.layout.fragment_schedule) {
    ...
    private val reminderScheduler by lazy { ReminderScheduler(requireContext()) }
    ...
    private fun initViews(root: View) {
        tvCurrentWeek = root.findViewById(R.id.tv_current_week)
        btnPreviousWeek = root.findViewById(R.id.btn_previous_week)
        btnNextWeek = root.findViewById(R.id.btn_next_week)
        btnMore = root.findViewById(R.id.btn_more)
        scheduleContainer = root.findViewById(R.id.schedule_container)
        ...
        titleWeekDay = root.findViewById(R.id.tv_title_week_day)
        weekDateLabels = listOf(
            root.findViewById(R.id.tv_date_mon),
            root.findViewById(R.id.tv_date_tue),
            root.findViewById(R.id.tv_date_wed),
            root.findViewById(R.id.tv_date_thu),
            root.findViewById(R.id.tv_date_fri),
            root.findViewById(R.id.tv_date_sat),
            root.findViewById(R.id.tv_date_sun)
        )
        weekDayLabels = listOf(
            root.findViewById(R.id.tv_day_mon),
            root.findViewById(R.id.tv_day_tue),
            root.findViewById(R.id.tv_day_wed),
            root.findViewById(R.id.tv_day_thu),
            root.findViewById(R.id.tv_day_fri),
            root.findViewById(R.id.tv_day_sat),
            root.findViewById(R.id.tv_day_sun)
        )
        dayColumnLayouts = listOf(
            root.findViewById(R.id.layout_day_mon),
            root.findViewById(R.id.layout_day_tue),
            root.findViewById(R.id.layout_day_wed),
            root.findViewById(R.id.layout_day_thu),
            root.findViewById(R.id.layout_day_fri),
            root.findViewById(R.id.layout_day_sat),
            root.findViewById(R.id.layout_day_sun)
        )
    ...
    private fun setupListeners() {
        ...
        fabQuickNote.setOnClickListener {
            ...
            if (course == null) {
                Toast.makeText(requireContext(), "当前没有进行中的课程", Toast.LENGTH_SHORT).show()
            } else {
    ...
    private fun updateSelectedDayHighlight() {
        ...
        val dayColor = ContextCompat.getColor(requireContext(), R.color.text_primary)
        val dateColor = ContextCompat.getColor(requireContext(), R.color.text_secondary)
    ...
    private fun showCreateSemesterDialog() {
        if (!::settingsRepository.isInitialized) {
            Toast.makeText(requireContext(), "学期数据尚未就绪", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        val pickDateButton = Button(requireContext()).apply {
            text = "选择日期"
        }
    ...
    private fun handleExportUri(uri: Uri) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val success = requireContext().contentResolver.openOutputStream(uri)?.use { output ->
    ...
    private fun showCourseWorkspaceDialog(course: Course) {
        if (!::workspaceRepository.isInitialized) {
            Toast.makeText(requireContext(), "工作区尚未准备好", Toast.LENGTH_SHORT).show()
            return
        }
        val dialog = BottomSheetDialog(requireContext())
    ...
    private fun showNoteInputDialog(
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("保存") { _, _ ->
                ...
                if (content.isBlank()) {
                    Toast.makeText(requireContext(), "笔记内容不能为空", Toast.LENGTH_SHORT).show()
    ...
    private fun showAddTaskDialog(course: Course, onSaved: () -> Unit) {
        ...
        val pickButton = Button(requireContext()).apply {
            text = "选择日期"
        }
    ...
    private fun saveAttachment(
        ...
        if (course.id <= 0) {
            Toast.makeText(requireContext(), "课程信息无效", Toast.LENGTH_SHORT).show()
            return
        }
        val attachment = CourseAttachmentEntity(
            semesterId = course.semesterId,
        ...
        lifecycleScope.launch {
            ...
            onSaved()
            Toast.makeText(requireContext(), "已添加附件", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun saveNote(course: Course, content: String, onSaved: () -> Unit) {
        ...
        lifecycleScope.launch {
            ...
            onSaved()
            Toast.makeText(requireContext(), "已保存笔记", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openAttachment(attachment: CourseAttachmentEntity) {
        when (attachment.type) {
            ...
            CourseAttachmentEntity.TYPE_MEDIA -> openMediaAttachment(attachment)
            else -> Toast.makeText(requireContext(), "无法打开该附件", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openMediaAttachment(attachment: CourseAttachmentEntity) {
        ...
        try {
            ...
            Toast.makeText(requireContext(), "无法打开该媒体", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openPdfAttachment(attachment: CourseAttachmentEntity) {
        ...
        try {
            ...
            Toast.makeText(requireContext(), "未找到可打开PDF的应用", Toast.LENGTH_SHORT).show()
        }
    }
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("删除") { _, _ ->
                lifecycleScope.launch {
                    ...
                    workspaceRefresh?.invoke()
                    Toast.makeText(requireContext(), "附件已删除", Toast.LENGTH_SHORT).show()
                }
            }
            .setNegativeButton("取消", null)
            .show()
